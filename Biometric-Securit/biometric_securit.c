...
static volatile angular_position_t angle = 0;
...
void BiometricSecurity_SetServoPosition(void);
void BiometricSecurity_CheckFingerprint(void);
void BiometricSecurity_ControlLed(uint8_t green_state);
void BiometricSecurity_LcdPrint(char* text, uint16_t length);
...
void BiometricSecurity_MsgHandler(service_t *service, msg_t *msg)
{
    if ((msg->header.cmd == IO_STATE)&&(RoutingTB_TypeFromID(msg->header.source) == STATE_TYPE))
    {
        ...
        {
            if ((!down_last_state)&(down_last_state != msg->data[0]))
            {
                angle = AngularOD_PositionFrom_deg(0);
                BiometricSecurity_SetServoPosition();
    ...
    if (RoutingTB_TypeFromID(msg->header.source) == FINGERPRINT_TYPE)
    {
        ...
        case CHECK:
            if (msg->data[0])
            {
                if (AngularOD_PositionTo_deg(angle) == 90)
                {
                    BiometricSecurity_SetServoPosition();
...
void BiometricSecurity_SetServoPosition(void)
{
    if (RoutingTB_IDFromType(SERVO_MOTOR_TYPE)>0)
    {
        ...
        BiometricSecurity_ControlLed(angle/90);
    }
    else 
    {
...
/******************************************************************************
 * @brief Function to check fingerprint
 * @param None
 * @return None
 ******************************************************************************/
void BiometricSecurity_CheckFingerprint(void)
